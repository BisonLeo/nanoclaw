...
export class QQChannel implements Channel {
  ...
  private handleDispatch(eventType: string, data: any): void {
    logger.info({ eventType, content: data?.content, timestamp: data?.timestamp }, 'QQ: Dispatch event received');
    switch (eventType) {
      case 'READY': {
  ...
  private handleC2CMessage(data: any): void {
    ...
    const attachments = this.attachmentsSummary(data.attachments);
    const content = [textContent, attachments].filter(Boolean).join(' ') || '[Empty message]';
    const msgId = data.id || '';
  ...
  private handleGroupMessage(data: any): void {
    ...
    const textContent = this.cleanContent(data.content || '');
    const attachments = this.attachmentsSummary(data.attachments);
    let content = [textContent, attachments].filter(Boolean).join(' ') || '[Empty message]';
    const msgId = data.id || '';
    const timestamp = data.timestamp
      ? new Date(data.timestamp).toISOString()
      : new Date().toISOString();
